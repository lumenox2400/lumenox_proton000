...
class LumeProton00:
    ...
    def overtaking_process(self, username_chosen, password_chosen, type_chosen=None):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                page.goto(self.page_id, wait_until="domcontentloaded", timeout=100000)
                print("Network activity:", page.evaluate("() => performance.getEntries().length"))
            ...
            if filtered_date < self.appointment_date:
                try:
                    ...
                    if self.type_appointment == 'Visa (Entrevista + Biométricos)' or new_appointment:
                        ...
                        # Created all possible combinations (interview × biometrics)
                        df_combos = df_filtered.assign(key=1).merge(
                            df_filtered_bios.assign(key=1),
                            on="key",
                            suffixes=("_int", "_bio")
                        ).drop("key", axis=1)
                        ...
                        print("Combinaciones viables encontradas:", len(df_combos_ok))
                        print(df_combos_ok[["date_int", "date_bio"]].head(10))

                        if df_combos_ok.empty:
                            self.final_msj = f"{self.final_msj} | No hay fechas disponibles para biometricos que cumplan las reglas"
                            return self.final_msj
                        
                        # Iterate over the valid combos to find the first one that meets the rules
                        for idx, row in combos_validas.iterrows():
                            try:
                                interview_date = row["date_int"]
                                bios_date = row["date_bio"]

                                # Select interview date
                                filtered_date = interview_date
                                page.evaluate("(el) => el.removeAttribute('readonly')", calendar_input)
                                calendar_input.evaluate(
                                    """(el, value) => {
                                        el.value = value;
                                ...
                                # Select first hour for interview date
                                time_select = page.wait_for_selector(
                                    '//*[@id="appointments_consulate_appointment_time_input"]',
                                    state='visible', timeout=30000
                                )
                                ...
                                # Select biometrics date
                                filtered_bios_date = bios_date
                                page.evaluate("""
                                    ({ selector, value }) => {
                                ...
                                # Select first hour for biometrics date
                                ...
                                try:
                                    ...
                                    self.final_msj += f" | Intento con {filtered_date} / {filtered_bios_date} exitoso"
                                    self.reschedule_success = True
                                except Exception as e:
                                ...
                                print(f"Falló combinación {idx}: {e}")
                                continue  # Try next combo
    ...
    def run(self):
        ...
        # If null then exit
        if self.username_chosen is None:
            print("No users available for processing. Exiting.")
            self.final_msj = f"{self.final_msj} | No hay usuarios activados para realizar el proceso"
